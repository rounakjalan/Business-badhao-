...
const VARIANT_CLASSES: Record<Variant, string> = {
  gradient: "bg-gradient-to-br from-bb-indigo to-bb-violet text-white shadow-sm hover:opacity-90 hover:shadow-md",
...
export function DashButton({ variant = "ghost", className = "", ...props }: DashButtonProps) {
  return (
    <button
      className={`bb-press inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all disabled:cursor-not-allowed disabled:opacity-40 ${VARIANT_CLASSES[variant]} ${className}`}
      {...props}
    />
  );
}
