...
import { CampaignsIcon, ProspectsIcon, SearchIcon } from "@/components/ui/icons";

export type CampaignProspectSummary = {
  id: string;
  name: string;
  status: string;
...
export function ProspectCampaignPickerClient({
  campaigns,
  unassignedCount,
}: {
  campaigns: CampaignProspectSummary[];
  ...
  if (campaigns.length === 0 && unassignedCount === 0) {
    return (
      <div className="bb-animate-fade-in flex flex-1 flex-col gap-5 p-4 sm:p-6">
        <PageHeader title="Prospects" description="Review prospects discovered through your campaigns." />
        <DarkEmptyState
          icon={CampaignsIcon}
          title="No campaigns yet"
          description="Create a campaign to start discovering prospects. Once it finds businesses, they'll show up here."
          action={
    ...
    <div className="bb-animate-fade-in flex flex-1 flex-col gap-5 p-4 sm:p-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bb-stagger grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((c) => (
            <Link key={c.id} href={`/prospects?campaign=${c.id}`} className="bb-stagger-item block">
              <DarkCard className="bb-lift flex h-full flex-col justify-between gap-4 p-5">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="font-jetbrains text-2xl font-semibold text-bb-text">{c.prospectCount.toLocaleString("en-IN")}</span>
                  <span className="text-xs text-bb-text-3">{c.prospectCount === 1 ? "prospect" : "prospects"}</span>
                </div>
                <div className="flex items-center justify-between border-t border-bb-navy-3 pt-3">
                  ...
                  <span className="bb-press text-sm font-medium text-bb-indigo transition-colors hover:text-bb-indigo-3">View Prospects →</span>
                </div>
              </DarkCard>
            </Link>
          ))}
      ...
      {unassignedCount > 0 ? (
        <Link href="/prospects?campaign=unassigned" className="block">
          <DarkCard className="bb-lift flex items-center justify-between gap-4 p-5">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-bb-navy-3 text-bb-text-3">
                <ProspectsIcon className="h-4.5 w-4.5" />
              </div>
              <div>
                <h3 className="text-sm font-semibold text-bb-text">Unassigned Prospects</h3>
