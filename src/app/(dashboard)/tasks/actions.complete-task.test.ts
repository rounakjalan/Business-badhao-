...
// Regression test for the audit finding: completeTask relied on RLS alone,
// with no application-level organization check — unlike every other write
// in this codebase. Proves the fix: the update is now explicitly scoped to
// the caller's own organization_id, and nothing is touched at all when
...
describe("completeTask", () => {
  ...
  it("scopes the update to the caller's own organization, alongside the task id", async () => {
    ...
    expect(eqCalls).toContainEqual({ column: "id", value: "task-1" });
